...
class SnakeRenderer:
    def __init__(self, cell_size=40, render_fps=60, moves_per_second=10):
        ...
        # Previous body positions for interpolation
        self.prev_body = None
    ...
    def _init_window(self, width, height):
        ...
        if not self.window_initialized:
            ...
            rl.init_window(window_width, window_height, "Snake Game")
    ...
    def render(self, state: GameState):
        ...
        if self.current_state is None:
            ...
            self.prev_body = deque(self.current_state.snake.body)
    ...
    def _render_frame(self, alpha):
        ...
        state = self.current_state if self.current_state is not None else self.next_state
        if state is None:
            return
    ...
    def _draw_snake_smooth(self, alpha):
        """Draw snake with smooth interpolation between current and next state"""
        ...
        if self.next_state is not None and self.next_state.snake.body:
            ...
            for i in range(len(next_body) - 1, -1, -1):
                ...
                self._draw_segment_smooth(interp_x, interp_y, is_head, direction, is_tail)
        else:
            ...
            for i in range(len(curr_body) - 1, -1, -1):
                ...
                x, y = curr_body[i]
                self._draw_segment_smooth(float(x), float(y), is_head, direction if is_head else None, is_tail)
    
    def _draw_segment_smooth(self, x, y, is_head, direction, is_tail):
        ...
        if is_head:
            rl.draw_rectangle_rounded(
                rl.Rectangle(px, py, size, size), 0.5, 8, self.SNAKE_COLOR
            )
            ...
            rl.draw_rectangle_rounded(
                rl.Rectangle(px, py, size, size), roundness, 8, self.SNAKE_COLOR
            )
    ...
    def _draw_ui(self, state):
        ...
        rl.draw_text(f"{state.score}", 60, 25, 28, self.WHITE)
        ...
        rl.draw_text(f"{len(state.snake.body)}", 160, 25, 28, self.WHITE)
    ...
    def _draw_game_over(self, state):
        ...
        rl.draw_text(text, (self.window_width - text_width) // 2, 
                    self.window_height // 2 - 80, 64, self.WHITE)
        ...
        rl.draw_text(score_text, (self.window_width - score_width) // 2, 
                    self.window_height // 2 - 10, 36, self.WHITE)
        
        # Restart hint
        restart_text = "Press R to restart"
        restart_width = rl.measure_text(restart_text, 24)
        rl.draw_text(restart_text, (self.window_width - restart_width) // 2,
                    self.window_height // 2 + 40, 24, self.WHITE)
        
        # Quit hint
        quit_text = "Press Q or ESC to quit"
        quit_width = rl.measure_text(quit_text, 20)
        rl.draw_text(quit_text, (self.window_width - quit_width) // 2,
                    self.window_height // 2 + 70, 20, rl.Color(200, 200, 200, 255))
